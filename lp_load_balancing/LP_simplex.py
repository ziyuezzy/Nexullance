...
def Solve_load_balancing(path_dict, edge_list, traffic_matrix=None, _integer=False, _solver='highs'):
    ...
    ## 1. constraints for the variables y1, y2,.., ym
    ...
    A = np.vstack( ( A, np.hstack((A_combinations, A_y, A_Y))))

    b = np.zeros(2*num_links)

    n_variables=A.shape[1]
    n_constraints=A.shape[0]
    assert(n_variables==num_paths+num_links+1)
    assert(n_constraints==2*num_links)
    ...
    result = linprog(c, A_ub=A, b_ub=b, A_eq=Aeq, b_eq=beq, bounds=bounds, method=_solver, options={'disp': True})
