...
def profile(config: tuple, traffic_pattern: str, _shift: int):
    ...
    # adapt the traffic scaling factor to 3x saturation
    traffic_scaling = 3.0/max(max_local_link_load, max_remote_link_load)
    ...
    # calibrate to 10x load for nexu_it, because its parameters are optimized for 10x load
    traffic_scaling_it = 10.0/max(max_local_link_load, max_remote_link_load)
    M_EPs_it = traffic_scaling_it * M_EPs
    M_R_it = gl.convert_M_EPs_to_M_R(M_EPs_it, config[0], EPR)

    nexu_it = Nexullance_IT(_network.nx_graph, M_R_it, Cap_remote)
    ...
    Lremote_NEXU_IT=nexu_it.get_result_max_link_load()/traffic_scaling_it
