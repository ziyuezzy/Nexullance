...
# def Solve_load_balancing_half_ILP(path_dict, edge_list, traffic_matrix=[], _solver=0, _verbose=0):
#     #LP solver options:
#     '''
#     0: Automatic (solver chooses the method)
#     1: Primal simplex
...
#                 if len(paths)>1: 
#                     normalization_constr[(s, d)]+=0.5*path_prob[unique_path_id]
#                 for i in range(len(path) - 1):
#                     vertex1, vertex2 = path[i], path[i + 1]
#                     link_load_constr[(vertex1, vertex2)]+=0.5*path_prob[unique_path_id]*traffic_matrix[s][d]
#                 unique_path_id+=1
...
#             for path in paths:
#                 if len(paths)>1:
#                     all_weighted_paths[(s, d)].append( (path, 0.5*path_prob[unique_path_id].x) )
#                 else:
#                     all_weighted_paths[(s, d)].append( (path, 1.0) )
#                 unique_path_id+=1
#         for (u, v), load in link_load_var.items():
