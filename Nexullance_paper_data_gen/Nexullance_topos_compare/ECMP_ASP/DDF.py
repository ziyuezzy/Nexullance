...
def main():
    ...
    with open(filename, 'w', newline='') as csvfile:
        ...
        for V, D in configs:
            ...
            traffic_pattern = "shift_quater"
            M_EPs = gl.generate_shift_traffic_pattern(V, EPR, EPR*V//4)
            remote_link_flows, local_link_flows = _network.distribute_M_EPs_on_weighted_paths(ECMP_ASP, EPR, M_EPs)
            max_remote_link_load = np.max(remote_link_flows)/Cap_remote
            max_local_link_load = np.max(local_link_flows)/Cap_local
            # adapt the traffic scaling factor to 10x saturation
            ...
            traffic_pattern = "diagonal_quater"
            M_EPs = gl.generate_diagonal_traffic_pattern(V, EPR, EPR*V//4)
            remote_link_flows, local_link_flows = _network.distribute_M_EPs_on_weighted_paths(ECMP_ASP, EPR, M_EPs)
            max_remote_link_load = np.max(remote_link_flows)/Cap_remote
            max_local_link_load = np.max(local_link_flows)/Cap_local
            # adapt the traffic scaling factor to 10x saturation
